...
def parse(statement, root_exp=None):
    ...
    clauses_to_parsers: "OrderedDict[SQLClause, OrderedDict[Type[ExtendedSyntax], typing.Tuple[TriggerFunc, ParserFunc]]]" = RegistryUtils.groupRegistryByClause(
        extended_syntax_registry
    )
...
def parse_select(relation, statement):
    columns = parse(statement, root_exp=lambda tokens: select_core_exp(tokens))
