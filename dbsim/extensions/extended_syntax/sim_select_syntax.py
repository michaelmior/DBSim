...
class Vector(Const):
    ...
    def __init__(self, vector: Vec):
        ERROR_IF_NOT_INSTANCE_OF(
            vector,
            Vec,
            "Vector can only be initialized with {} ({} received)".format(
                getClassNameOfClass(Vec), type(vector)
            ),
        )
        self.const = vector
...
def getVecDistance(v1: Vec, v2: Vec, metric: Metric = Metric.EUC) -> float:
    ERROR_IF_NOT_EQ(
        v1.shape,
        v2.shape,
        "could not compute the distance between vectors with different shapes {} and {}".format(
            v1.shape, v2.shape
        ),
    )
...
def convert_values_to_vectors(df_column):
    ...
    if df_column.dtype == object and all(
        value.startswith("[") and value.endswith("]")
        for value in df_column[:num_detect_samples]
    ):
...
class SimSelectionSyntax(ExtendedSyntax):
    ...
    def trigger_simselect(self, tokens: TokenList) -> bool:
        try:
            ...
            self.I_am_triggered = tokens[0] == self.simselect_keyword or (
                tokens[0] == "select" and "to" in tokens[:index_of_from_clause]
            )
            return self.I_am_triggered
        except ValueError:
            # no 'from' in tokens, i.e., there are no nested queries
            self.I_am_triggered = tokens[0] == self.simselect_keyword or (
                tokens[0] == "select" and "to" in tokens
            )
            return self.I_am_triggered
        ...
        if tokens[:1] == ["where"]:
            ...
            if self.simselect_detected:
                ERROR_IF_FALSE(
                    ...
                    or to_keyword_detected_in_where_clause,
                    "simselect requires at least one 'to' expression, zero found.",
                    ExtendedSyntaxError,
                )
            if (
                to_keyword_detected_in_where_clause
                or self.to_keyword_detected_in_select_clause
                or self.simselect_detected
            ):
                ...
                relation: SimSelectionOp = self.extended_where_core_expr(
                    tokens, relation
                )
            else:
