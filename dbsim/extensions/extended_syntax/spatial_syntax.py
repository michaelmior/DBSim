...
def isInside(p: Point, c: Circle):
    ERROR_IF_NOT_INSTANCE_OF(
        p,
        Point,
        "the first argument of 'isInside' must be a Point({} received)".format(type(p)),
        SQLSyntaxError,
    )
    ERROR_IF_NOT_INSTANCE_OF(
        c,
        Circle,
        "the second argument of 'isInside' must be a Circle({} received)".format(
            type(c)
        ),
        SQLSyntaxError,
    )
            ...
            if token.isdigit() or token == ".":
                ...
                try:
                    ...
                    raise SQLSyntaxError(
                        "invalid vector element: '{}'".format(cur_element)
                    )
                if cur_coordinate == 0:
                    x = cur_element
                    ...
                    raise SQLSyntaxError(
                        "Higher dimensional points than 2D are not supported."
                    )
                cur_element = ""
            if len(tokens) == 0:
                raise SQLSyntaxError("missing closing '#'")
...
def parse_circle(tokens: TokenList) -> Circle:
    ...
    if token == "{":
        ...
        if token != ",":
            raise SQLSyntaxError(
                "unexpected token '{}' after center claim in circle.".format(token)
            )
        token = tokens.pop(0)
        while token != "}":
            if not token.isdigit() and token != ".":
                raise SQLSyntaxError(
                    "invalid symbol '{}' found in circle.".format(token)
                )
            radius += token
            if len(tokens) == 0:
                raise SQLSyntaxError("missing closing '}'")
...
def parse_inside_exp(tokens: TokenList) -> Expr:
    ...
    if len(tokens):
        ERROR_IF_NOT_EQ(
            tokens[0], "inside", "keyword 'inside' not found", ParsingFailure
        )
        tokens.pop(0)
...
class SpatialSyntax(ExtendedSyntax):
    ...
    _extended_data_types_: typing.Dict[typing.Type[Const], str] = {
        RelPoint: "POINT",
        RelCircle: "CIRCLE",
    }
    ...
    _extended_predicate_op_executors_: typing.Dict[
        typing.Type[SimpleOp], typing.Callable[[typing.Any], typing.Callable]
    ] = {InsideOp: get_inside_op_executor()}
    _extended_relation_op_schema_: typing.Dict[
        typing.Type[SuperRelationalOp],
        typing.Callable[[SuperRelationalOp, "DataSet"], "Schema"],
    ] = {SpatialSelectionOp: schema_interpreter.schema_from_relation}
    _extended_relation_op_executors_: typing.Dict[
        typing.Type[SuperRelationalOp], typing.Callable[[typing.Any], typing.Callable]
    ...
    def trigger_spatialselect(self, tokens: TokenList) -> bool:
        try:
            ...
            self.I_am_triggered = tokens[0] == self.spatialselect_keyword or (
                tokens[0] == "select" and "inside" in tokens[:index_of_from_clause]
            )
            return self.I_am_triggered
        except ValueError:
            # no 'from' in tokens, i.e., there are no nested queries
            self.I_am_triggered = tokens[0] == self.spatialselect_keyword or (
                tokens[0] == "select" and "inside" in tokens
            )
            return self.I_am_triggered
        ...
        if tokens[:1] == ["where"]:
            ...
            if self.spatialselect_detected:
                ERROR_IF_FALSE(
                    ...
                    or inside_keyword_detected_in_where_clause,
                    "spatialselect requires at least one 'inside' expression, zero found.",
                    ExtendedSyntaxError,
                )
            if (
                inside_keyword_detected_in_where_clause
                or self.inside_keyword_detected_in_select_clause
                or self.spatialselect_detected
            ):
                ...
                relation: SpatialSelectionOp = self.extended_where_core_expr(
                    tokens, relation
                )
            else:
    ...
    def extended_where_core_expr(
        self, tokens: TokenList, relation: Expr
    ) -> SpatialSelectionOp:
