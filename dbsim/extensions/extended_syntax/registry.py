            ...
            clause_parsers=OrderedDict(
                {
                    ...
                    SQLClause.WHERE: (
                        "trigger_simselect_where",
                        "parse_simselect_where",
                    ),
                }
            ),
            entry_points=[
            ...
            clause_parsers=OrderedDict(
                {
                    ...
                    SQLClause.WHERE: (
                        "trigger_spatialselect_where",
                        "parse_spatialselect_where",
                    ),
                }
            ),
            entry_points=[
...
def validateRegistry():
    ...
    for syntax_name, reg_entry in registry.items():
        ERROR_IF_NOT_INSTANCE_OF(
            reg_entry.clause_parsers,
            OrderedDict,
            "RegEntry.clause_parsers must be an OrderedDict(should not be {}).".format(
                type(reg_entry.clause_parsers)
            ),
            RegistryError,
        )
