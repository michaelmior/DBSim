...
def addDataTypes(
    new_nodetypes_and_datatypes: typing.Dict[typing.Type[Const], FieldType]
):
    ...
    for nodetype in new_nodetypes_and_datatypes:
        if not issubclass(nodetype, Const):
            raise ExtensionInternalError(
                "schema_interpreter.addDataTypes only accepts subclasses of 'Const' ({} received)".format(
                    type(nodetype)
                )
            )
...
def removeDataTypes(
    removed_nodetypes_and_datatypes: typing.Dict[typing.Type[Const], FieldType]
):
    global ConstNodeToDataType
...
def addRelationOps(
    new_op_types_and_schema_funcs: typing.Dict[
...
def removeRelationOps(
    removed_op_types_and_schema_funcs: typing.Dict[
        typing.Type[SuperRelationalOp],
        typing.Callable[[SuperRelationalOp, "DataSet"], Schema],
    ]
) -> None:
...
def resolve_schema_for_node(dataset, loc, op):
    func_for_updating_op_schema = op_type_to_update_schema_func.get(
        type(op), update_op_schema(schema_from_relation)
    )
    ...
    if expr_type == Var:
        ...
        if lhs_field.type != rhs_field.type:
            ...
            return lhs_field.new(
                name="?column?".format(
                    expr_type.__name__, lhs_field.name, rhs_field.name
                )
            )
...
def field_from_const(expr):
    ...
    return Field(name="?column?", type=ConstNodeToDataType[type(expr)])
...
def replace_schema(op, schema):
    return op.new(
        schema=schema
    )  # 'new' is a method that returns a deep copy of the existing object with the specified attrs overwritten, see immutable.py for details.
