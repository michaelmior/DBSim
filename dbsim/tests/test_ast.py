...
def test_equalExprs_all_cases():
  ...
  """
    ...
    assert equalExprs(
        ast, query2.operations, ignore_schema=True, match_loadop_and_relation=True
    )
    assert not equalExprs(
        ast, query2.operations, ignore_schema=False, match_loadop_and_relation=True
    )
    assert not equalExprs(
        ast, query2.operations, ignore_schema=True, match_loadop_and_relation=False
    )
    assert not equalExprs(
        ast, query2.operations, ignore_schema=False, match_loadop_and_relation=False
    )
  ...
  """
    ...
    for node, parent, idx in traverseWithParent(ast, None, 0, "dfs"):
        ...
        if i == 0:
            ...
            assert (
                isinstance(parent, dfs_truth_node_parent_seq[i][0])
                and idx == dfs_truth_node_parent_seq[i][1]
            )
        i += 1
    i = 0
    for node, parent, idx in traverseWithParent(ast, None, 0, "bfs"):
        ...
        if i == 0:
            ...
            assert (
                isinstance(parent, bfs_truth_node_parent_seq[i][0])
                and idx == bfs_truth_node_parent_seq[i][1]
            )
        i += 1
