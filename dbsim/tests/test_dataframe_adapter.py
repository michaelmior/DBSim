...
def test_df_adapter_count():
    col = "employee_id"
    truth_query = truth_dataset.query(
        "select count({col}) from employees".format(col=col)
    )
    ...
    assert (
        truth_res[0][0] == truth_adapter.get_relation("employees").size()
        and truth_res == df_res
    )


def test_df_adapter_min_max():
    col = "employee_id"
    truth_query_min = truth_dataset.query(
        "select min({col}) from employees".format(col=col)
    )
    truth_query_max = truth_dataset.query(
        "select max({col}) from employees".format(col=col)
    )
    ...
    assert (
        truth_res_min[0][0] == true_min
        and truth_res_max[0][0] == true_max
        and truth_res_max == df_res_max
        and truth_res_min == df_res_min
    )
...
def test_df_adapter_concat():
    col = "full_name"
    truth_query = truth_dataset.query(
        "select concat({col}) from employees".format(col=col)
    )
    truth_res = truth_query.get_pretty_results()
...
def test_df_adapter_sum():
    col = "employee_id"
    truth_query = truth_dataset.query(
        "select sum({col}) from employees".format(col=col)
    )
    truth_res = truth_query.get_pretty_results()
