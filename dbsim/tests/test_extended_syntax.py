...
def test_SimSelectionSyntax():
  ...
  """
    ...
    assert (
        isinstance(ast, ProjectionOp)
        and isinstance(ast.relation, LoadOp)
        and len(ast.exprs) == 1
        and isinstance(ast.exprs[0], Vector)
    )
  ...
  """
    ...
    assert (
        ...
        and isinstance(ast.relation, LoadOp)
        and len(ast.exprs) == 1
        and isinstance(ast.exprs[0], ToOp)
    )
  ...
  """
    ...
    ast = parse_statement(
        "simselect vector to [1, 2, 3, 4] from employees_with_vectors"
    )
    """
  ast should look like:
  ...
  """
    assert (
        ...
        and isinstance(ast.exprs[0], ToOp)
        and isinstance(ast.relation, LoadOp)
        and ast.relation.name == "employees_with_vectors"
    )
  ...
  """
    assert (
        ...
        and len(ast.exprs) == 1
        and isinstance(ast.exprs[0], Var)
    )
    ast = parse_statement(
  ...
  """
    ...
    exe_res = [row[0] for row in dataset.query(sql).execute()]
    assert len(exe_res) == 1 and exe_res[0] == euclidean(
    ...
    df_truth["dist"] = df_truth["vector"].apply(
        lambda v: euclidean(v, np.array([1, 2, 3, 4]))
    )
...
def test_ExtendedSyntax_mixture():
        ...
        for row in Query(dataset, parse_statement(sql)).execute():
            ...
            for col_idx, col in enumerate(row):
                if isinstance(col, np.ndarray):
                    assert isinstance(
                        truth_res[row_idx][col_idx], np.ndarray
    ...
    assert (
        ...
        and isinstance(ast.relation, SelectionOp)
        and isinstance(ast.relation.bool_op, GtOp)
        and isinstance(ast.relation.relation, SimSelectionOp)
        and isinstance(ast.relation.relation.bool_op, LtOp)
        and isinstance(ast.relation.relation.bool_op.lhs, ToOp)
    ...
    assert (
        ...
        and isinstance(ast.relation, SpatialSelectionOp)
        and isinstance(ast.relation.bool_op, InsideOp)
        and isinstance(ast.relation.relation, LoadOp)
        and ast.relation.relation.name == "points"
    )
    ...
    assert (
        isinstance(ast, SimSelectionOp)
        and isinstance(ast.bool_op, Or)
        and isinstance(ast.bool_op.lhs, LtOp)
        and isinstance(ast.bool_op.lhs.lhs, ToOp)
        and isinstance(ast.bool_op.rhs, InsideOp)
        and isinstance(ast.relation, LoadOp)
        and ast.relation.name == "employees_with_vectors"
    )
    exec_sql(
        sql_21,
        [
            (1234, "Tom Tompson", date(2009, 1, 17), -1, (), np.array([1, 2, 3, 4])),
            (
                4567,
                "Sally Sanders",
                date(2010, 2, 24),
    ...
    assert (
        ...
        and len(ast.exprs) == 2
        and ast.exprs[0].path == "employees_with_vectors.employee_id"
        and ast.exprs[1].path == "points.pid"
        and isinstance(ast.relation, SpatialSelectionOp)
        and isinstance(ast.relation.bool_op, And)
        and isinstance(ast.relation.bool_op.lhs, GtOp)
        and isinstance(ast.relation.bool_op.rhs, InsideOp)
        and isinstance(ast.relation.relation, JoinOp)
        and isinstance(ast.relation.relation.bool_op, EqOp)
        and isinstance(ast.relation.relation.left, SimSelectionOp)
        and isinstance(ast.relation.relation.right, LoadOp)
        and isinstance(ast.relation.relation.left.bool_op, LtOp)
        and isinstance(ast.relation.relation.left.bool_op.lhs, ToOp)
        and isinstance(ast.relation.relation.left.relation, LoadOp)
        and ast.relation.relation.left.relation.name == "employees_with_vectors"
        and ast.relation.relation.right.name == "points"
    )
