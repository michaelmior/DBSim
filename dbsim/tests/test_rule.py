...
def test_FilterMergeRule():
  ...
  """
    ...
    assert equalExprs(
        ast, resolved_plan, ignore_schema=True, match_loadop_and_relation=True
    )
...
Relation: employees  Relation: employees_2
  """
    ...
    assert isinstance(resolved_plan, SelectionOp) and Predicate.fromRelationOp(
        resolved_plan
    ).equalToExprByStr(
        "employees.employee_id > 1234 AND employees.employee_id = employees_2.employee_id"
    )
    assert isinstance(resolved_plan.relation, JoinOp) and Predicate.fromRelationOp(
        resolved_plan.relation
    ).equalToExprByStr("true")
    assert isinstance(resolved_plan.relation.left, Relation) and isinstance(
        resolved_plan.relation.right, Relation
    )
...
Relation: employees
  """
    assert isinstance(equiv_plan, SelectionOp) and Predicate.fromRelationOp(
        equiv_plan
    ).equalToExprByStr("employees.employee_id = employees_2.employee_id")
    assert isinstance(equiv_plan.relation, JoinOp) and Predicate.fromRelationOp(
        equiv_plan.relation
    ).equalToExprByStr("true")
    assert isinstance(
        equiv_plan.relation.left, SelectionOp
    ) and Predicate.fromRelationOp(equiv_plan.relation.left).equalToExprByStr(
        "employees.employee_id > 1234"
    )
    ...
    assert (
        Query(dataset, resolved_plan, resolve_op_schema=False).get_pretty_results()
...
Relation: employees
  """
    assert isinstance(equiv_plan, JoinOp) and Predicate.fromRelationOp(
        equiv_plan
    ).equalToExprByStr("employees.employee_id = employees_2.employee_id")
    assert isinstance(equiv_plan.left, SelectionOp) and Predicate.fromRelationOp(
        equiv_plan.left
...
Relation: employees                                         Relation: employees_2
  """
    ...
    assert isinstance(equiv_plan.right, SelectionOp) and Predicate.fromRelationOp(
        equiv_plan.right
...
def test_SimSel_Sel_Swap_Rule():
  ...
  """
    assert (
        getClassNameOfInstance(equiv_plan.relation) == "SimSelectionOp"
