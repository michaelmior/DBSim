...
class RuleOperand(object):
    ...
    def __init__(self, ast_root_class, child_operand_list):
        ERROR_IF_FALSE(
            ...
            "Parameter 'ast_node_class' is not a class ({} received).".format(
                type(ast_root_class)
            ),
        ...
        ERROR_IF_NOT_INSTANCE_OF(
            child_operand_list,
            list,
            "Parameter 'child_operand_list' is not a list ({} received).".format(
                type(child_operand_list)
            ),
            PlannerInternalError,
        )
        ERROR_IF_COMPARISON_FALSE(
            len(child_operand_list),
            2,
            Comparison.LEQ,
            "'child_operand_list' is required to include at most 2 elements ({} received).".format(
                len(child_operand_list)
            ),
            PlannerInternalError,
        )
    ...
    def matches(self, ast_root: Union[RelationalOp, BinRelationalOp]):
        ...
        while i < len(self.children):
            ...
            if not isinstance(self.children[i], AnyMatchOperand) and not isinstance(
                self.children[i], NoneOperand
            ):
                return False
            i += 1
        return True
