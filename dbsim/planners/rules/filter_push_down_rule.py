...
class FilterPushDownRule(Rule):
    ...
    def _transformImpl(
        self, ast_root: Expr, inplace: bool = False
        ...
        ERROR_IF_FALSE(
            ast_root.isResolved(), "unresolved plan received", PlannerInternalError
        )
        if inplace:
            copy_ast = ast_root
        else:
        ...
        decorrelated_predicates: Dict[
            typing.Tuple[int], Predicate
        ] = PredicateUtils.decorrelateAnd(
            Predicate.fromRelationOp(selection), [join.left, join.right]
        )
        ...
        for related_node_indices in decorrelated_predicates:
            ...
            if len(related_node_indices) == 0:
                ...
                join.left = SelectionOp(
                    join.left, predicate.toExpr(), schema=join.left.schema.copy()
                )
                join.right = SelectionOp(
                    join.right, predicate.toExpr(), schema=join.right.schema.copy()
                )
                ...
                selection = SelectionOp(
                    join, predicate.toExpr(), schema=selection.schema.copy()
                )
