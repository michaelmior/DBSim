...
class Planner(object):
    ...
    def addRule(self, rule: Rule) -> bool:
        ERROR_IF_NOT_INSTANCE_OF(
            rule,
            Rule,
            "The rule to be added is not a Rule instance ({} received)".format(
                type(rule)
            ),
        )
        if self.locked:
    ...
    def optimizeQuery(self, query: Query) -> Query:
        ...
        ERROR_IF_NOT_INSTANCE_OF(
            query,
            Query,
            "'{}.optimizeQuery(query)' requires a Query object as input ({} received)".format(
                getClassNameOfInstance(self), type(query)
            ),
        )
