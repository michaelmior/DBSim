...
class LogicalCost(object):
    ...
    def refineCostFactors(cls, node: Expr, is_predicate: bool) -> None:
        ...
        ERROR_IF_NONE(
            refined_cost_factor,
            "{} has no 'cost_factor' attribute.".format(getClassNameOfInstance(node)),
        )
        if hasattr(node, "exprs"):
            ...
            for predicate in predicates:
                ERROR_IF_NOT_INSTANCE_OF(
                    predicate,
                    SimpleOp,
                    "requires predicate to be a SimpleOp instance ({} received)".format(
                        type(predicate)
                    ),
                )
    ...
    def getCost(cls, plan: Expr, dataset: DataSet) -> "LogicalCost":
        ...
        """
        ERROR_IF_FALSE(
            plan.isResolved(), "requires a resolved plan (unresolved plan received)"
        )
        ctx = {"dataset": dataset}
