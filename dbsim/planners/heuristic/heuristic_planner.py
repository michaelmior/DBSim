...
class HeuristicPlanner(Planner):
    ...
    def __init__(
        self,
        match_order: PlanMatchOrder = PlanMatchOrder.DepthFirstOrder,
        max_limit: int = 100,
    ) -> None:
        super().__init__()
        ...
        ERROR_IF_FALSE(
            max_limit > 0,
            "the limit for maximum number of rule applications must be positive ({} received).".format(
                max_limit
            ),
        )
    ...
    def applyRule(self, node: Expr, rule: Rule) -> Expr:
        ...
        ERROR_IF_NOT_INSTANCE_OF(
            node,
            Expr,
            "'{}.applyRule{}' requires an Expr object as the first argument ({} received)".format(
                getClassNameOfInstance(self),
                inspect.signature(self.applyRule),
                type(node),
            ),
        )
        ERROR_IF_FALSE(
            node.isResolved(),
            "'{}.applyRule{}' requires a resolved expression tree as input (unresolved tree received)".format(
                getClassNameOfInstance(self), inspect.signature(self.applyRule)
            ),
            PlannerInternalError,
        )
        ERROR_IF_NONE(
            self.root, "planner hasn't setup any plan root.", PlannerInternalError
        )
        ...
        ERROR_IF_NONE(
            best_plan, "Something wrong with the equivalent plans", PlannerInternalError
        )

        if equalResolvedExprs(node, best_plan):
            # the current rule did not change the original plan
            logger.debug(
                "Applying rule '{}' did not change the original plan.".format(
                    getClassNameOfInstance(rule)
                )
            )
            return None
    ...
    def findBestPlan(self, plan: Expr) -> Expr:
        ...
        ERROR_IF_NOT_INSTANCE_OF(
            plan,
            Expr,
            "'{}.findBestPlan(plan)' requires an Expr object as input ({} received)".format(
                getClassNameOfInstance(self), type(plan)
            ),
        )
        ERROR_IF_FALSE(
            plan.isResolved(),
            "'{}.findBestPlan{}' requires a resolved expression tree as input (unresolved tree received)".format(
                getClassNameOfInstance(self), inspect.signature(self.findBestPlan)
            ),
            PlannerInternalError,
        )
        ...
        while num_rules_applied <= self.max_num_applications:
            ...
            if node is not None:
                ...
                for rule in self.rule_seq:
                    ...
                    debugger.debug(
                        "applyRule on node {} returned {}".format(
                    ...
                    if new_node is not None:
                        ...
                        if node is self.root:
                            ...
                            debugger.debug(
                                "the whole plan rooted at node {} transformed to new plan rooted at {}".format(
                                    str(node), str(new_node)
                                )
                            )
                            ...
                            debugger.debug(
                                "a sub-plan rooted at node {} (parent: {}) transformed to new sub-plan rooted at {}".format(
                                    str(node), str(parent), str(new_node)
                                )
                            )
                            debugger.debug(
                                "set {} as child #{} of the parent {}".format(
                                    str(new_node), child_idx, str(parent)
                                )
                            )
        ...
        raise PlannerInternalError(
            "'{}.findBestPlan()' reached somewhere invalid.".format(
                getClassNameOfInstance(self)
            )
        )
