...
AggConfTable = [
    ...
    Aggregate(
        "min", min, Field(name="min", type=FieldType.INTEGER), float("Inf"), None
    ),
    Aggregate(
        "max", max, Field(name="max", type=FieldType.INTEGER), float("-Inf"), None
    ),
    ...
    Aggregate(
        "sum",
        lambda state, next: state + next,
        Field(name="sum", type=FieldType.INTEGER),
        0,
        None,
    ),
    Aggregate(
        "concat",
        lambda state_str, cur_str: state_str + cur_str,
        Field(name="concat", type=FieldType.STRING),
        "",
        None,
    ),
]
