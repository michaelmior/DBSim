...
def removeSyntaxSymbol(symbols: str) -> int:
    ...
    for symbol in symbols:
        ...
        if symbol_idx >= 0:
            ExtensibleTokens.SYMBOLS = (
                ExtensibleTokens.SYMBOLS[:symbol_idx]
                + ExtensibleTokens.SYMBOLS[symbol_idx + 1 :]
            )
            count_success += 1
    return count_success
...
def addClauseKeywords(keywords: Dict[SQLClause, str]):
    ...
    for clause in keywords:
        ERROR_IF_NOT_INSTANCE_OF(
            clause,
            SQLClause,
            "'{}' is not a {} instance.".format(
                type(clause), getClassNameOfClass(SQLClause)
            ),
        )
        ERROR_IF_FALSE(
            clause in sql_clause_keywords,
            "Attempted to add keywords into non-supported SQL clause '{}'.".format(
                type(clause)
            ),
        ...
        ERROR_IF_NOT_INSTANCE_OF(
            ...
            str,
            "'{}' is not a {}.".format(
                type(keywords[clause]), getClassNameOfClass(str)
            ),
        )
...
def addPredParsers(
    ...
    parsers: Dict[PredExprLevel, Callable],
    block_error: bool = not BLOCK_ERROR,
):
    ...
    for level in parsers:
        ERROR_IF_NOT_INSTANCE_OF(
            ...
            PredExprLevel,
            "'{}' is not a {} instance.".format(
                type(level), getClassNameOfClass(PredExprLevel)
            ),
        )
        ERROR_IF_FALSE(
            level in predicate_parsers,
            "Adding predicate parsers of new predicate levels is not supported currently.",
            ExtensionInternalError,
        )
        predicate_parsers[level].add(
            getClassNameOfClass(syntax), parsers[level], block_error
        )
...
def resetPredParsers():
    global predicate_parsers
    predicate_parsers = {
        PredExprLevel.OR: ParsersBundle(
            [("StandardSyntax", or_exp, not BLOCK_ERROR)], PredExprLevel.OR
        ),
        PredExprLevel.AND: ParsersBundle(
            [("StandardSyntax", and_exp, not BLOCK_ERROR)], PredExprLevel.AND
        ),
        PredExprLevel.COMP: ParsersBundle(
            [("StandardSyntax", comparison_exp, not BLOCK_ERROR)], PredExprLevel.COMP
        ),
        PredExprLevel.ADD: ParsersBundle(
            [("StandardSyntax", additive_exp, not BLOCK_ERROR)], PredExprLevel.ADD
        ),
        PredExprLevel.MUL: ParsersBundle(
            [("StandardSyntax", multiplicative_exp, not BLOCK_ERROR)], PredExprLevel.MUL
        ),
        PredExprLevel.UNARY: ParsersBundle(
            [("StandardSyntax", unary_exp, not BLOCK_ERROR)], PredExprLevel.UNARY
        ),
        PredExprLevel.VALUE: ParsersBundle(
            [("StandardSyntax", value_exp, not BLOCK_ERROR)], PredExprLevel.VALUE
        ),
        PredExprLevel.VAR: ParsersBundle(
            [("StandardSyntax", var_exp, not BLOCK_ERROR)], PredExprLevel.VAR
        ),
        PredExprLevel.TUPLE: ParsersBundle(
            [("StandardSyntax", tuple_exp, not BLOCK_ERROR)], PredExprLevel.TUPLE
        ),
        PredExprLevel.FUNC: ParsersBundle(
            [("StandardSyntax", function_exp, not BLOCK_ERROR)], PredExprLevel.FUNC
        ),
    }
...
class ParsersBundle:
    ...
    def __init__(
        self,
        parsers: List[typing.Tuple[Callable[[TokenList], Expr], bool]],
        level: PredExprLevel,
    ):
    ...
    def __call__(self, arg1, arg2=None) -> Expr:
            ...
            try:
                ...
                if not block_error:
                    if not isinstance(e, ParsingFailure):
                        logger.error(
                            "{} raised an error: (see the info below)".format(
                                syntax_name
                            )
                        )
                        raise e
        ...
        ERROR_IF_NONE(
            res,
            "could not parse tokens {} by any parser of level {}".format(
                tokens, self.predicate_level
            ),
            SQLSyntaxError,
        )
    ...
    def add(
        self,
        syntax_name: str,
        parser: Callable[[TokenList], Expr],
        block_error: bool = not BLOCK_ERROR,
    ):
        if len(self.parsers) == 1:
            ...
            self.parsers = (
                self.parsers[:-1]
                + [(syntax_name, parser, block_error)]
                + [self.parsers[-1]]
            )
...
def case_when_core_exp(tokens):
    ...
    while tokens and tokens[0].lower() == "when":
        ...
        expr = parse_or(tokens)
        condition_map = dict(condition=condition, expr=expr)
...
def projection_op(relation, columns):
    if (
        len(columns) == 1
        and isinstance(columns[0], SelectAllExpr)
        and columns[0].table is None
    ):
...
def join_source(
    tokens,
    clauses_to_parsers: "OrderedDict[SQLClause, OrderedDict[Type[ExtendedSyntax], typing.Tuple[TriggerFunc, ParserFunc]]]" = OrderedDict(),
):
    """
    ...
    while tokens and tokens[0] in (",", "join", "left"):
        ...
        if join_type == "left":
            if tokens[0] == "outer":
                tokens.pop(0)
            ERROR_IF_NOT_EQ(
                tokens[0], "join", "Missing keyword 'join' after 'left'", SQLSyntaxError
            )
            tokens.pop(0)
            op = LeftJoinOp
        else:
    ...
    while tokens and tokens[0] != ")":
        if tokens[0] == "(":
            ...
            if isinstance(expr, Var):
                ...
                raise ValueError(
                    "Only constants, relationame or select queries allowed"
                )
...
def select_stmt(
    ...
    parse_select_by_standard, parse_from_by_standard, parse_where_by_standard = (
        True,
        True,
        True,
    )
    ...
    triggered_clause_parsers_for_select: "OrderedDict[SQLClause, typing.Tuple[Type[ExtendedSyntax], ParserFunc]]" = getTriggeredSQLClauseParsersInfo(
        tokens, syntax_instances, clauses_to_parsers, (SQLClause.SELECT,)
    )
    ...
    if len(triggered_clause_parsers_for_select) > 0:
        ...
        parse_syntax = syntaxFuncFromName(syntax_instances[syntax], parser_func_name)
    ...
    triggered_clause_parsers_for_where: "OrderedDict[SQLClause, typing.Tuple[Type[ExtendedSyntax], ParserFunc]]" = getTriggeredSQLClauseParsersInfo(
        tokens, syntax_instances, clauses_to_parsers, (SQLClause.WHERE,)
    )
...
def getTriggeredSQLClauseParsersInfo(
    ...
    clause_parsers_triggered: "OrderedDict[SQLClause, typing.Tuple[Type[ExtendedSyntax], ParserFunc]]" = (
        OrderedDict()
    )
    ...
    for sql_clause in clauses_to_parsers:
        ...
        for syntax in clauses_to_parsers[sql_clause]:
            is_triggered = syntaxFuncFromName(
                syntax_instances[syntax], clauses_to_parsers[sql_clause][syntax][0]
            )
            if is_triggered(tokens) == True:
                ...
                if sql_clause in clause_parsers_triggered:
                    ...
                    logger.warning(
                        "Multiple clause parsers are triggered for the same SQL clause ({} parsers of {} and {})".format(
                            sql_clause,
                            getClassNameOfClass(
                                clause_parsers_triggered[sql_clause][0]
                            ),
                            getClassNameOfClass(syntax),
                        )
                    )
                else:
                    clause_parsers_triggered[sql_clause] = (
                        syntax,
                        clauses_to_parsers[sql_clause][syntax][1],
                    )
