...
def ERROR_IF_COMPARISON_FALSE(
    arg1, arg2, comp: Comparison, msg=None, exception_type=ValueError
):
    ERROR_IF_DIFF_TYPES(arg1, arg2)
    if (
        comp == Comparison.LE
        and not (arg1 < arg2)
        or comp == Comparison.LEQ
        and not (arg1 <= arg2)
        or comp == Comparison.GE
        and not (arg1 > arg2)
        or comp == Comparison.GEQ
        and not (arg1 >= arg2)
        or comp == Comparison.EQ
        and not (arg1 == arg2)
        or comp == Comparison.NEQ
        and not (arg1 != arg2)
    ):
        if msg is None:
...
def minus(set1: set, set2: set) -> set:
    ERROR_IF_NOT_INSTANCE_OF(
        set1, set, "'set1' is required to be a set ({} received)".format(type(set1))
    )
    ERROR_IF_NOT_INSTANCE_OF(
        set2, set, "'set2' is required to be a set ({} received)".format(type(set2))
    )
    return set1 - set2
