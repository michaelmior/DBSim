...
class PredicateUtils(object):
    ...
    @classmethod
    def _decorrelateAnd(
        cls, predicate: Predicate, nodes: List[Expr]
        ...
        if cls._isAnd(predicate):
            pred_lhs, pred_rhs = Predicate(predicate.toExpr().lhs), Predicate(
                predicate.toExpr().rhs
            )
    ...
    @classmethod
    def _groupDecorrelatedAnd(
        cls, decorrelatedAnd: Dict["Predicate", List[int]]
        ...
        for pred in decorrelatedAnd:
            ...
            if related_node_indices in groups:
                groups[related_node_indices] = cls._mergeByAnd(
