...
def addExecutor(
    ...
    if op_class in dst_executor_table:
        logger.warning(
            "You are overwriting an existing executor: {} - {}".format(
                getClassNameOfClass(op_class), op_exe_func
            )
        )
...
def removeExecutor(
    ...
    if op_class in dst_executor_table and op_exe_func is dst_executor_table[op_class]:
        ...
        logger.warning(
            "You are removing a non-existing executor: {} - {}".format(
                getClassNameOfClass(op_class), op_exe_func
            )
        )
    ...
    for nodetype in extended_data_types:
        if not issubclass(nodetype, Const):
            raise ExtensionInternalError(
                "compilers.local.addDataTypes only accepts subclasses of 'Const' ({} received)".format(
                    type(nodetype)
                )
            )
...
def removeDataTypes(removed_data_types: typing.Dict[typing.Type[Const], str]):
    ...
    for nodetype in removed_data_types:
        if not issubclass(nodetype, Const):
            raise ExtensionInternalError(
                "compilers.local.removeDataTypes only accepts subclasses of 'Const' ({} received)".format(
                    type(nodetype)
                )
            )
...
def computeCost(
    ...
    op_node: Expr,
    input1: typing.Tuple,
    input2: typing.Tuple = None,
    aggregate_two_inputs=None,
) -> None:
    """
    ...
    if aggregate_two_inputs is not None and input2 is not None:
        ...
        ctx[stat_field_in_ctx][num_input_rows_and_cost_factor_field].append(
            (aggregate_two_inputs(input1, input2), op_node.cost_factor)
        )
    else:
        ...
        ctx[stat_field_in_ctx][num_input_rows_and_cost_factor_field].append(
            (num_rows_input1 + num_rows_input2, op_node.cost_factor)
        )
    ...
    return walk(
        ...
        visit_with(
            ...
            (
                isa_op,
                relational_op,
            ),  # here the logical plan node is transformed to its physical executable
...
def join_op(left_join, dataset, operation):
    ...
    def join(ctx):
        ...
        computeCost(
            ctx,
            operation,
            tuple(input_rows_left),
            tuple(input_rows_right),
        ...
        for pos, agg in pos_and_aggs:
            ...
            if sig is not None:
                ...
                if num_input_params == 0:
                    raise Exception(
                        "Invalid aggregate definition: no input parameters accepted by the aggregate. \
