...
class DataSet(object):
    ...
    def add_aggregate(self, name, func, returns, initial, finalize=None):
        if name in self.udfs:
            ...
            raise ValueError(
                "'{}' is already registered as a user-defined function.".format(name)
            )
    ...
    def add_function(self, name, function, returns=None):
        if name in self.aggregates:
            ...
            raise ValueError(
                "'{}' is already registered as an aggregate function.".format(name)
            )
    ...
    def adapter_for(self, relation_name: str):
        for adapter in self.adapters:
            if adapter.has(relation_name):
                return adapter
        raise RuntimeError(
            "Relation '{}' not found in Dataset '{}'".format(relation_name, self.name)
        )
    ...
    def execute(self, query, *params, **kw):
        ...
        default_ctx = {"dataset": self, "params": params}
