...
class Schema(ImmutableMixin):
    ...
    def get_field(self, path):
        ...
        if len(fields) == 1:
            ...
            raise FieldNotFoundError('No such field "{}"'.format(path))
        else:
            raise AmbigousFieldError('Field "{}" is ambigous'.format(path))
    ...
    def copy(self) -> "Schema":
        return Schema(fields=deepcopy(self.fields), name=self.name)
