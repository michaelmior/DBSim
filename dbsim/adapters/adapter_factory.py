...
class AdapterFactory(object):
    ...
    def addDataTypeConverter(cls, extended_syntax: Type, datatype_converter: Callable):
        ERROR_IF_FALSE(
            ...
            "method 'addDataTypeConverter' only accepts an ExtendedSyntax subclass as the first argument ('{}' received)".format(
                type(extended_syntax)
            ),
        )
        if extended_syntax in cls.extendedSyntaxDataTypeConverters:
            logger.warning(
                "The datatype converter of ExtendedSyntax '{}' is already registered.".format(
                    extended_syntax
                )
            )
    ...
    def removeDataTypeConverter(cls, extended_syntax: Type):
        ERROR_IF_FALSE(
            isSubclassByClassName(extended_syntax, "ExtendedSyntax"),
            "method 'removeDataTypeConverter' only accepts an ExtendedSyntax subclass as the first argument ('{}' received)".format(
                type(extended_syntax)
            ),
        )
        if extended_syntax in cls.extendedSyntaxDataTypeConverters:
            logger.warning(
                "The datatype converter of ExtendedSyntax '{}' is not registered.".format(
                    extended_syntax
                )
            )
    ...
    @classmethod
    def convertType(
        cls, df_column: pd.Series
    ) -> typing.Tuple[pd.Series, field.FieldType]:
        """
        ...
        raise RuntimeError(
            "No matched FieldType to pandas dtype '{}'".format(df_col_dtype)
        )
    ...
    def fromFile(cls, filepath: str, ds_name: str = None) -> Adapter:
        ...
        ERROR_IF_FALSE(
            ...
            "File type '{}' not supported currently (supported types: {})".format(
                ext, ", ".join(list(cls.supported_filetypes))
            ),
        )
        if ds_name is None:
