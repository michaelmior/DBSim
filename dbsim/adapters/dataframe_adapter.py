    ...
    def __init__(self, **tables):
        ...
        for name, table in tables.items():
            if isinstance(table, dict):
                ...
                raise RuntimeError(
                    "Invalid table setup for '{}', please input the table using Python dict and specify its schema".format(
                        name
                    )
                )

            self._tables[name] = DataFrameTable(self, name, schema=schema, df=df)
